...
interface RouterInterface{
  public function add($method, $route, $target, $name);
  public function has($name);
  public function generate($name, $params);
  public function match($request_url, $request_method);
}
...
trait RouterTraits{
  ...
  private $base_path;
  public function base_path($base_path = false){
    if($base_path !== false){
	...
	public function match($request_url = null, $request_method = null) {
		...
		foreach($this->routes as $handler) {
			...
			if ($_route === '*') {
				...
				$match = preg_match('`' . substr($_route, 1) . '`', $request_url, $params);
			} else {
				$route = null;
				$regex = false;
				$j = 0;
